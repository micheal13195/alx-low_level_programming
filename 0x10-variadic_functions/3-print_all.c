...
/**
 * print_float - this helper function print a float
...
/**
 * print_int - this helper print an integer
...
/**
 * print_str - this function print a string
	...
	while (format && format[format_index])
	{
		...
		while (fn_list[fn_index].format)
		{
			if (format[format_index] == fn_list[fn_index].format)
			{
				...
				bytes += fn_list[fn_index].fn(arguments);
				break;
			}
			++fn_index;
		}
